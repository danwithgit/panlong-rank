...
缺失历史数据可以继续补拉，但需要按数据类型区分：

- 指数历史：可以通过 AKShare 指数日线补齐，数据质量为 `backfilled`。
- 个股历史：可以通过 AKShare A 股日线慢速补齐，受海外网络和上游限制影响，失败会退避重试。
- 板块历史：免费接口无法稳定提供完整历史板块成分和板块行情；当前只能由已回填个股按板块聚合，质量标记为 `partial`。
- 非交易日：不应补数据；交易日历确认的周末/节假日任务会标记为 `skipped`。

如果未来要让历史周榜、环比完全可靠，建议接入授权行情源，或增加一个可稳定返回历史板块全量成分和板块行情的 provider。
