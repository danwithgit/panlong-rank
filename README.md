...
默认 compose 会启动 PostgreSQL、Redis 和应用。`DATA_PROVIDER` 默认设为 `auto`，优先使用 AKShare，失败时回退样例数据。
...
- 当前默认 `DATA_PROVIDER=auto`：AKShare 全量接口失败时，会用东方财富单指数接口更新上证指数，并用样例板块/个股排行兜底，数据源标记为 `eastmoney_index_sample_rankings`。
